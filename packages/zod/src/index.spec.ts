...
describe("withObjectKeys", () => {
  it("exposes the zod object schema's keys via the ~standard.objectKeys surface", () => {
    const schema = withObjectKeys(z.object({ orgId: z.string(), userId: z.string() }));
  ...
  it("keeps the schema usable as a Standard Schema", () => {
    const schema = withObjectKeys(z.object({ userId: z.string() }));
  ...
  it("throws an actionable error when the schema is not a zod object", () => {
    expect(() => withObjectKeys(z.string())).toThrow(/zod object schema/);
  });

  it("composes into a message contract with working type inference", () => {
    const contract = defineMessageContract({
      consumerSchema: withObjectKeys(z.object({ type: z.literal("user.created"), id: z.string() })),
